...
def store_documents_node(state: FileIngestionState) -> Dict[str, Any]:
    """
    ...
    Args:
        state: FileIngestionState containing documents and resource id
    ...
    try:
        ...
        logger.info(f"[STORE_DOCUMENTS_NODE] Storing {len(chunked_documents)} documents in vector database for resource {resource_id}")
        ...
        logger.error(f"[STORE_DOCUMENTS_NODE] Failed to store documents in vector database: {str(e)}")
        return {
